...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Get current academic year based on the current academic session
    ...
    `;
      ...
      if (academicYearResult.rows.length === 0) {
        ...
        return res.status(400).json({
          message:
            "No active academic session found. Cannot process leave request.",
        });
      }
    ...
    `;
      ...
      if (balanceResult.rows.length === 0) {
        await client.query("ROLLBACK");
        return res.status(400).json({
          message: `No leave balance found for this teacher for the ${academicYear} academic year`,
        });
      }
    ...
    try {
      ...
      if (balancesResult.rows.length === 0) {
        ...
        try {
          ...
          const newBalancesResult = await pool.query(balancesQuery, [
            teacherId,
            academicYear,
          ]);
  ...
  async (req, res) => {
    try {
      ...
      const result = await pool.query(
        "SELECT restore_teacher_status_after_leave()"
      );
