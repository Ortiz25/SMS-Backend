 ...
 * @access  Private
 */
router.get('/students/:classId', authorizeRoles('admin', 'teacher', 'staff'), async (req, res) => {
  try {
    const { classId } = req.params;
          ...
          if (existingResult.rows.length > 0) {
            ...
            result = await client.query(
              `UPDATE exam_results
               ...
               RETURNING *`,
              [marks, letterGrade, points, is_absent, student_id, exam_schedule_id]
            );
          } else {
            // Insert new result
            result = await client.query(
              `INSERT INTO exam_results (student_id, exam_schedule_id, marks_obtained, grade, points, is_absent)
               VALUES ($1, $2, $3, $4, $5, $6)
               RETURNING *`,
              [student_id, exam_schedule_id, marks, letterGrade, points, is_absent]
            );
          }
 ...
 * @access  Private
 */
router.get('/student-grades/:studentId/:examId', authorizeRoles('admin', 'teacher', 'staff'), async (req, res) => {
  try {
 ...
 * @access  Private
 */
router.get('/report-card/:studentId/:academicSessionId', authorizeRoles('admin', 'teacher', 'staff'), async (req, res) => {
  try {
    ...
    const teacherComments = commentQuery.rows.length > 0 ? commentQuery.rows[0].comments : null;
 ...
 * @access  Private
 */
router.post('/batch-report-cards', authorizeRoles('admin', 'teacher', 'staff'), async (req, res) => {
  try {
 ...
 * @access  Private
 */
router.post('/teacher-comments', authorizeRoles('admin', 'teacher', 'staff'), async (req, res) => {
  try {
    const { student_id, examination_id, comments } = req.body;
    
    if (!student_id || !examination_id) {
      return res.status(400).json({ msg: 'Student ID and Examination ID are required' });
    }
    ...
    if (summaryCheck.rows.length === 0) {
      return res.status(404).json({ msg: 'No result summary found for this student and examination' });
    }
 ...
 * @access  Private
 */
router.get('/sessions', authorizeRoles('admin', 'teacher', 'staff'), async (req, res) => {
  try {
 ...
 * @access  Private
 */
router.get('/current-session', authorizeRoles('admin', 'teacher', 'staff'), async (req, res) => {
  try {
