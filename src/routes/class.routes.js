...
const classValidation = [
    ...
    body('curriculum_type').isIn(['CBC', '844'])
...
router.get('/:id/students', async (req, res, next) => {
    try {
        ...
        const result = await classModel.getClassStudents(req.params.id, page, limit);
...
router.post('/:id/subjects',
    ...
    async (req, res, next) => {
        try {
            const result = await classModel.assignSubjects(req.params.id, req.body.subject_ids);
            res.json(result.rows);
        } catch (error) {
            next(error);
        }
...
router.post('/:id/teacher',
    authorizeRoles('admin'),
    body('teacher_id').isInt().withMessage('Valid teacher ID is required'),
    async (req, res, next) => {
        try {
            const result = await classModel.assignClassTeacher(req.params.id, req.body.teacher_id);
            if (!result.rows.length) {
...
router.get('/subjects', authorizeRoles('admin', 'teacher', 'staff'), async (req, res, next) => {
    try {
      ...
      if (level && curriculum_type) {
        // Get subjects for the specific class level and curriculum type
        query = `
          SELECT DISTINCT 
            s.id, 
            s.name, 
          ...
          ORDER BY s.name
        `;
        queryParams = [curriculum_type, level];
      } else {
        // Get all subjects
        query = `
          SELECT 
